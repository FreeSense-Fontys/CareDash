...
async function defaultGenerator(
    ...
    const bloodpressure = interpolateBloodPressure(EXAMPLE_DATA, start, stop, interval);
    ...
    for (let index = 0; index < heartRateSignal.length; index++) {
        ...
        const timeDiffS = timeDiffOptions[Math.floor(Math.random() * timeDiffOptions.length)];
        const timeDiffMoptions = [0, 1, 2]
        const timeDiffM = timeDiffMoptions[Math.floor(Math.random() * timeDiffMoptions.length)];
        const timeDiffMprocessed = parseInt(HR.time.substring(0, 2)) >= 23
            ? "00"
            : (timeDiffM + parseInt(HR.time.substring(3, 5))).toString().padStart(2, '0');

        const measurement: {
            vitals: { name: string; value: number; sqiStatus: number; timestamp: string }[];
            scheduleTags: string[];
            ppgFileToken: string;
        } = {
            vitals: [],
        ...
        if (HR) {
            measurement.vitals.push({
                ...
                value: HR.HR,
                sqiStatus: 1,
                timestamp: convertTimeToISO(generateForDate, `${HR.time.substring(0, 2)}:${timeDiffMprocessed}`)
            });
        }
        if (BP) {
            measurement.vitals.push({
        ...
        const post = await exh.data.documents.create('wearable-observation', measurement);
    }

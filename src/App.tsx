...
function App() {
    ...
    const createSchemaTiming = async () => {
        await exh.data.schemas.create({
            name: 'preset-timing',
            description:
                'This is a basic object for the timing of the wearable',
            defaultLimit: 20,
            maximumLimit: 20,
        })
    }

    const addPropertiesToTiming = async () => {
        await exh.data.properties.create('preset-timing', {
            name: 'value',
            configuration: {
                type: 'number',
            },
    ...
    const addDataToTiming = async () => {
        await exh.data.documents.create('preset-timing', {
            value: 15,
            timingType: 'Interval',
            time: 'Minutes',
        })
    ...
    await exh.data.documents.create('wearable-preset', {
        name: 'POTS',
        carepathId: '682ee8958bd54e4ac40e3d02',
        vitals: ['HR', 'DBP', 'SBP'],
        timings: ['68431cfc8bd54e1b0c0e3e27'],
        alerts: ['6835623f8bd54e629f0e3d2d', '6836d1a08bd54e0e100e3d59'],
    });
    }
    ...
    const createSchema = async () => {
        ...
        // createSchemaTiming();
        // addPropertiesToTiming();
        // addDataToTiming();
        // removeSchema('presetTiming')
        // createSchemaPreset();
